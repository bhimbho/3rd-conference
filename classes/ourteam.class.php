...
class ourteam extends db
{
	...
	**/
	 ...
	 * Update Team Profile
	 */
	public function update_team($team_id, $name, $position, $order, $pic)
	{
		$query = $this->execute("UPDATE `our_team` SET `team_name` = ?, `team_pos` = ?,`team_order` = ?, `team_pic` = ? WHERE `team_id`=?", [$name, $position, $order, $pic, $team_id]);
        return ($query)? true : false;
	}
